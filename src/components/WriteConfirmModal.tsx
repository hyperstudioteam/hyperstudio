...
export function WriteConfirmModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[60] grid place-items-center bg-black/55"
      onClick={onCancel}
    >
      <div
        className="w-[460px] max-w-[92vw] rounded-lg border border-danger/40 bg-panel shadow-[0_24px_60px_rgba(0,0,0,.5)]"
      ...
      >
        ...
        <div className="flex flex-col gap-2.5 px-3.5 py-3">
          <p className="m-0 text-[11px] leading-[1.5] text-text">
            This connection is guarded. The statement below modifies data or
            schema.
          </p>

          <pre className="scrollbar-thin-app m-0 max-h-[160px] overflow-auto rounded-md border border-border bg-surface-deep p-2.5 font-mono text-[10px] leading-[1.5] whitespace-pre-wrap text-text">
            {sql}
          </pre>

          <label className="flex flex-col gap-[5px] text-[10px] font-[540] text-muted">
            Type <strong className="text-text-bright">{connectionName}</strong> to
            continue
            <input
        ...
        <footer className="flex justify-end gap-2 border-t border-border px-3.5 py-2.5">
          ...
          <button
            type="button"
            className="cursor-pointer rounded-[5px] border border-danger/50 bg-danger/10 px-3 py-1.5 text-[11px] font-semibold text-danger hover:border-danger hover:text-white disabled:opacity-40"
            disabled={!matches}
            onClick={onConfirm}
          >
            Run it
